...
必选:
  -i FILE   DIAMOND输出 (13列, 制表符分隔, 第13列包含TC编号)
...
validate_input_format() {
    log_info "验证输入文件格式..."
    local first_line
    first_line=$(grep -v '^#' "${INPUT}" | head -n 1) || true
...
EOF
    ...
    if [ "${KEEP_ALL}" = "yes" ]; then
        ...
        # FIX: sort 时 E-value 用科学计数法排序需要转为数值，这里用 awk 内置排序
    ...
    # FIX: 移除无效的 header 跳过逻辑 (tmp_final 不含表头)
